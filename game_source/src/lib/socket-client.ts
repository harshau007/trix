...
class SocketClient {
  ...
  makeMove(matchId: string, player: string, board: (number | null)[][], score: number, direction: string) {
    if (!this.socket?.connected) {
      throw new Error("Socket not connected")
    }

    console.log(`Making 2048 move: ${matchId}, player: ${player}, direction: ${direction}`)
